...
describe('Order Management Integration Tests', () => {
  let testData: any;
  let company2Data: any;

  beforeEach(async () => {
    testData = await setupCompanyWithUsers({ companyName: 'OrderTestCo', employeeCount: 1 });
    testData.employee = testData.employees![0]; // Add shorthand for first employee
    
    company2Data = await setupCompanyWithUsers({ companyName: 'Company2', employeeCount: 1 });
  ...
  describe('Order Creation', () => {
    describe('Happy Path - Create Order', () => {
      it('should create order with menu items for an open event', async () => {
        ...
        const restaurant = await createRestaurant({ companyId: testData.company.id });
        const menuItem1 = await createMenuItem({ restaurantId: restaurant.id, price: 10.5 });
        const menuItem2 = await createMenuItem({ restaurantId: restaurant.id, price: 5.0 });
        ...
        // Join event first
        await authenticatedRequest(app, testData.employee.token)
          .post(`/api/events/${event.id}/join`)
          .expect(201);

        // Create order
        const response = await authenticatedRequest(app, testData.employee.token)
      ...
      it('should create order with both custom order and menu items', async () => {
        const restaurant = await createRestaurant({ companyId: testData.company.id });
        const menuItem = await createMenuItem({ restaurantId: restaurant.id, price: 8.5 });
        const event = await createEvent({
          companyId: testData.company.id,
          createdById: testData.admin.id,
          restaurantId: restaurant.id,
    ...
    describe('Authorization - Create Order', () => {
      ...
      it('should reject order if event is from different company', async () => {
        const restaurant = await createRestaurant({ companyId: company2Data.company.id });
        const event = await createEvent({
  ...
  describe('Order Updates', () => {
    describe('Update Existing Order', () => {
      it('should update order with new items (replaces old items)', async () => {
        const restaurant = await createRestaurant({ companyId: testData.company.id });
        const menuItem1 = await createMenuItem({ restaurantId: restaurant.id, price: 10.0 });
        const menuItem2 = await createMenuItem({ restaurantId: restaurant.id, price: 15.0 });
        const event = await createEvent({
          companyId: testData.company.id,
          createdById: testData.admin.id,
          restaurantId: restaurant.id,
      ...
      it('should maintain one order per user per event (idempotency)', async () => {
        ...
        // Create order
        const firstOrder = await authenticatedRequest(app, testData.employee.token)
          .post(`/api/orders/${event.id}/orders`)
          .send({ customOrder: 'First order', totalAmount: 10.00 })
          .expect(201);

        // Create/update order again
        const secondOrder = await authenticatedRequest(app, testData.employee.token)
          .post(`/api/orders/${event.id}/orders`)
  ...
  describe('Order Deletion', () => {
    it('should delete own order from open event', async () => {
      ...
      expect(event.paymentMethod).toBe('INDIVIDUAL');

      await authenticatedRequest(app, testData.employee.token)
        .post(`/api/events/${event.id}/join`)
        .expect(201);

      const orderResponse = await authenticatedRequest(app, testData.employee.token)
        .post(`/api/orders/${event.id}/orders`)
        .send({ customOrder: 'Test order', totalAmount: 10.00 })
        .expect(201);
      ...
      // Verify deleted
      const deletedOrder = await prisma.order.findUnique({ where: { id: orderId } });
    ...
    it('should reject deleting another user\'s order', async () => {
      ...
      const orderResponse = await authenticatedRequest(app, testData.admin.token)
        .post(`/api/orders/${event.id}/orders`)
  ...
  describe('Payment Confirmation', () => {
    it('should confirm payment for own order', async () => {
      ...
      expect(storedOrder?.userId).toBe(testData.employee.id);
    ...
    it('should reject confirming payment for another user\'s order', async () => {
      ...
      expect(response.body).toHaveProperty('message', 'Only event creator can confirm payments');
    });
  ...
  describe('Company Isolation', () => {
    it('should not show orders from different companies', async () => {
      const restaurant1 = await createRestaurant({ companyId: testData.company.id });
      const restaurant2 = await createRestaurant({ companyId: company2Data.company.id });
      ...
      await authenticatedRequest(app, company2Data.employee.token)
        .post(`/api/events/${event2.id}/join`)
        .expect(201);
      ...
      const response1 = await authenticatedRequest(app, testData.employee.token)
        .get('/api/orders/me')
        .expect(200);
      ...
      const response2 = await authenticatedRequest(app, company2Data.employee.token)
        .get('/api/orders/me')
        .expect(200);
