...
### Phase 1 – Tenant Onboarding Guardrails
- **Goal**: Prevent cross-company registration and establish a verifiable join flow.
- **Scope**:
  - Replace raw `companyId` registration with one of:
    - Email-domain verification tied to company slug
    - Invite token generated by company admins
  - Add server-side throttles + audit events for failed join attempts.
  - Update validation + tests (backend `auth.controller`, `auth.validation`, integration suites).
  - UI updates for invite redeem pages + error messaging.
- **Deliverables**:
  - RFC describing invite flow
  - Integration tests covering accepted + rejected invite tokens
  - Frontend Register adjustments and new invite redemption view
...
## Next Steps
1. Draft an RFC for Phase 1 invitation flow (owners: Backend + Product).
2. Book design time for Phase 3 UX improvements.
3. Timebox spike on WebAuthn (Phase 4) to assess feasibility.
