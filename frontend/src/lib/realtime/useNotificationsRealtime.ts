...
const buildNotificationModel = (payload: RealtimeNotification): NotificationEvent => ({
  ...
  title: payload.title,
  body: payload.body,
  actor: (payload as any).actor ?? (payload.meta as any)?.actor ?? null,
  subject: (payload as any).subject ?? (payload.meta as any)?.subject,
  cta: (payload as any).cta ?? (payload.meta as any)?.cta,
  meta: payload.meta,
  event: undefined,
  order: undefined,
  user: undefined,
